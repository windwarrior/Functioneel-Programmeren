...
sprockell  prog  state  tick   
               | opc /= NoOp = State {dmem=dmem',regbank=regbank',pc=pc',cnd=cnd'} -- als de opcode iets anders is dan NoOp, dan moet het cnd veld geupdate worden
               | otherwise   = state {dmem=dmem',regbank=regbank',pc=pc'} -- Als de opcode NoOp was, dan was er dus een load/store/whatever, die iig geen verandering moet doen aan eht conditional veld
               where
